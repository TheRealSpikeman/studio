...
import { Download, Brain } from '@/lib/icons';
import { useToast } from '@/hooks/use-toast';
import type { QuizResult } from '@/types/dashboard';
import DOMPurify from 'isomorphic-dompurify';
import { format, parseISO } from 'date-fns';
import { nl } from 'date-fns/locale';
import { ResultsChart } from '@/components/quiz/quiz-steps/ResultsChart';
import { SingleScoreChart } from './SingleScoreChart';
...
export function AnonymousResultView({ result }: { result: QuizResult }) {
  const { toast } = useToast();

  const handlePdfDownloadClick = () => {
    toast({
        title: "Rapport Printen",
        description: `Gebruik de print-functie van uw browser (Ctrl/Cmd + P) om het rapport op te slaan als PDF.`,
      });
    window.print();
  };
  ...
  return (
    <div className="min-h-screen bg-muted/30 flex items-center justify-center p-4">
       <div className="absolute top-4 left-4 md:top-8 md:left-8 print-hide">
          <SiteLogo />
      </div>
