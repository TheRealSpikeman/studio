        ...
        <Card>
          ...
          <CardContent className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              />
              <FormField name="name" control={form.control} render={({ field }) => (<FormItem><FormLabel>Plannaam (Publiek)</FormLabel><FormControl><Input placeholder="Bijv. Gezins Gids" {...field} /></FormControl><FormMessage /></FormItem>)} />
            </div>
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <FormField name="shortName" control={form.control} render={({ field }) => (<FormItem><FormLabel>Korte Naam</FormLabel><FormControl><Input placeholder="Bijv. 1 Kind" {...field} /></FormControl><FormMessage /></FormItem>)} />
              <FormField name="tagline" control={form.control} render={({ field }) => (<FormItem><FormLabel>Tagline (optioneel)</FormLabel><FormControl><Input placeholder="Beste voor startende gezinnen" {...field} /></FormControl><FormMessage /></FormItem>)} />
            </div>
            <FormField name="description" control={form.control} render={({ field }) => (<FormItem><FormLabel>Beschrijving</FormLabel><FormControl><Textarea placeholder="Leg uit voor wie dit plan is..." {...field} rows={3} /></FormControl><FormMessage /></FormItem>)} />
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
               <FormField name="price" control={form.control} render={({ field }) => (<FormItem><FormLabel>Prijs per Maand</FormLabel><div className="relative"><Euro className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><FormControl><Input type="number" step="0.01" placeholder="15.00" {...field} value={field.value || ''} className="pl-8" /></FormControl></div><FormMessage /></FormItem>)} />
               <FormField name="yearlyDiscountPercent" control={form.control} render={({ field }) => (<FormItem><FormLabel>Jaarkorting (%)</FormLabel><div className="relative"><Percent className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><FormControl><Input type="number" min="0" max="100" placeholder="10" {...field} value={field.value || ''} className="pl-8"/></FormControl></div><FormMessage /></FormItem>)} />
               <FormField name="trialPeriodDays" control={form.control} render={({ field }) => (<FormItem><FormLabel>Proefperiode (dagen)</FormLabel><FormControl><Input type="number" min="0" placeholder="14" {...field} value={field.value || ''} /></FormControl><FormMessage /></FormItem>)} />
            </div>
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
               <FormField name="maxParents" control={form.control} render={({ field }) => (<FormItem><FormLabel>Max. Ouders</FormLabel><FormControl><Input type="number" min="0" placeholder="2" {...field} value={field.value ?? ''} /></FormControl><FormMessage /></FormItem>)} />
               <FormField name="maxChildren" control={form.control} render={({ field }) => (<FormItem><FormLabel>Max. Kinderen</FormLabel><FormControl><Input type="number" min="0" placeholder="1" {...field} value={field.value ?? ''} /></FormControl><FormMessage /></FormItem>)} />
            </div>
          </CardContent>
        </Card>
