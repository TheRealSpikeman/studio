...
export const getQuizById = async (id: string): Promise<QuizAdmin | null> => {
    // This is the corrected version that was previously in storageService
    if (!isFirebaseConfigured || !db) {
        console.warn("Firebase not configured, falling back to mock quiz data.");
    ...
    try {
        ...
        if (docSnap.exists()) {
            ...
            // Fallback for demo environments where the quiz might be in the mock data
            const allQuizzes = await getQuizzes();
            return allQuizzes.find(q => q.id === id) || null;
        }
    } catch (error) {
        console.error(`Error fetching quiz by id ${id}, falling back to mock data:`, error);
        const allQuizzes = await getQuizzes();
        return allQuizzes.find(q => q.id === id) || null;
    }
};