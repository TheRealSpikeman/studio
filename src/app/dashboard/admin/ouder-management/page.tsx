...
import { Search, PlusCircle, Users } from '@/lib/icons';
...
export default function OuderManagementPage() {
  ...
  useEffect(() => {
    // Load users from dummy data and filter for 'ouder' role
    setOuders(DUMMY_USERS.filter(u => u.role === 'ouder'));
  }, []);
  ...
  const confirmDeleteOuder = () => {
    if (selectedOuder) {
      setOuders(prev => prev.filter(s => s.id !== selectedOuder.id));
      toast({ title: "Ouder verwijderd", description: `Ouder ${selectedOuder.name} is verwijderd (simulatie).` });
      setSelectedOuder(null);
    }
    setIsDeleteModalOpen(false);
  };
  
  const handleSaveOuder = (updatedOuderData: User) => {
    if (selectedOuder) {
        setOuders(prevOuders => prevOuders.map(s => s.id === selectedOuder.id ? { ...s, ...updatedOuderData } : s));
        toast({ title: "Ouder bijgewerkt", description: `Gegevens voor ${updatedOuderData.name} zijn opgeslagen (simulatie).` });
    }
